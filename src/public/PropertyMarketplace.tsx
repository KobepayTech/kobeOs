...
function ShopHero({ site, shop, products, brand }: { site: MarketplaceData['site']; shop: Shop; products: number; brand: string }) {
  return <section className="bg-slate-950 text-white"><div className="mx-auto max-w-7xl px-4 py-10 sm:px-6"><a href="/" className="text-xs font-bold text-white/50">← Back to {site.name}</a><div className="mt-5 flex flex-col gap-5 sm:flex-row sm:items-center"><div className="grid h-20 w-20 place-items-center overflow-hidden rounded-3xl bg-white/10">{shop.business?.logoUrl ? <img src={publicAssetUrl(shop.business.logoUrl)} className="h-full w-full object-cover" /> : <Store className="h-8 w-8" />}</div><div><div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-[10px] font-black"><MapPin className="h-3 w-3" />{shop.floor?.name || 'Ground'} · {shop.publicCode}</div><h1 className="mt-3 text-3xl font-black sm:text-5xl">{shop.business?.name || shop.publicCode}</h1><p className="mt-2 text-sm text-white/55">{products} products available in this shop.</p></div><div className="sm:ml-auto flex gap-2">{shop.business?.phone && <a href={`tel:${shop.business.phone}`} className="inline-flex h-11 items-center gap-2 rounded-xl border border-white/15 px-4 text-xs font-black"><Phone className="h-4 w-4" /> Call</a>}<a href={`https://${shop.business?.publicSlug}.kobeapptz.com`} className="inline-flex h-11 items-center gap-2 rounded-xl px-4 text-xs font-black text-white" style={{ background: brand }}>Full shop <ArrowRight className="h-4 w-4" /></a></div></div></div></section>;
}
