...
interface LalaOffer { id: string; totalPrice: number; currency: string; message: string; expiresAt: string; hotel: { id: string; name: string; location: string } | null; room: { id: string; roomNumber: string; type: string; rate: number; currency: string } | null }
...
export default function Lala() {
  ...
  const [reverseRequestId, setReverseRequestId] = useState(() => localStorage.getItem('lala_reverse_request_id') || ''); const [offers, setOffers] = useState<LalaOffer[]>([]); const [offersLoading, setOffersLoading] = useState(false);
  ...
  const book = async () => { if (!selected) return; setLoading(true); setError(''); try { const token = await ensurePassport(); const result = await publicApi<{ booking: { id: string }; hotel: string; room: string; nights: number; pendingConfirmation?: boolean; message?: string }>('/lala-public/bookings', { method: 'POST', body: JSON.stringify({ hotelId: selected.hotel.hotel.id, roomId: selected.room.id, passportToken: token, checkIn: search.checkIn, checkOut: search.checkOut, guests: Number(search.guests) }) }); setBooking(result.pendingConfirmation ? `Saved safely by Lala backup as ${result.booking.id.slice(0, 8)}. Final room confirmation will happen automatically when the primary KobeOS path returns.` : `Booking ${result.booking.id.slice(0, 8)} received at ${result.hotel}, room ${result.room}.`); setSelected(null); } catch (e) { setError((e as Error).message); } finally { setLoading(false); } };
  const loadOffers = async (requestId = reverseRequestId, token = passportToken) => { if (!requestId || !token) return; setOffersLoading(true); setError(''); try { setOffers(await publicApi<LalaOffer[]>(`/lala-public/reverse-requests/${requestId}/offers?passportToken=${encodeURIComponent(token)}`)); } catch (e) { setError((e as Error).message); } finally { setOffersLoading(false); } };
  const reverse = async () => { setLoading(true); setError(''); try { const token = await ensurePassport(); const result = await publicApi<{ id: string; pendingConfirmation?: boolean }>('/lala-public/reverse-requests', { method: 'POST', body: JSON.stringify({ passportToken: token, destination: search.destination, checkIn: search.checkIn, checkOut: search.checkOut, guests: Number(search.guests), budget: Number(budget), currency: 'TZS' }) }); localStorage.setItem('lala_reverse_request_id', result.id); setReverseRequestId(result.id); setOffers([]); setBooking(result.pendingConfirmation ? `Request ${result.id.slice(0, 8)} saved by the independent backup. Hotels will receive it automatically when primary KobeOS reconnects.` : `Request ${result.id.slice(0, 8)} sent. Hotels can now offer you a room here.`); setReverseOpen(false); if (!result.pendingConfirmation) void loadOffers(result.id, token); } catch (e) { setError((e as Error).message); } finally { setLoading(false); } };
  const acceptOffer = async (offer: LalaOffer) => { if (!reverseRequestId || !passportToken) return; setOffersLoading(true); setError(''); try { const result = await publicApi<{ booking: { id: string }; hotel: string; room: string }>(`/lala-public/reverse-requests/${reverseRequestId}/offers/${offer.id}/accept`, { method: 'POST', body: JSON.stringify({ passportToken }) }); setBooking(`Booking ${result.booking.id.slice(0, 8)} confirmed from the offer at ${result.hotel}, room ${result.room}.`); localStorage.removeItem('lala_reverse_request_id'); setReverseRequestId(''); setOffers([]); } catch (e) { setError((e as Error).message); } finally { setOffersLoading(false); } };
  useEffect(() => { if (reverseRequestId && passportToken) void loadOffers(reverseRequestId, passportToken); }, [reverseRequestId, passportToken]);
  ...
  <main className="max-w-7xl mx-auto p-4 sm:p-6">{rows.some((row) => row.backupMode) && <div className="rounded-xl bg-amber-50 border border-amber-200 text-amber-800 p-3 mb-4 text-sm font-bold"><ShieldCheck className="inline h-4 w-4 mr-1" /> Lala is using its independent backup. Search data is a recent snapshot; new bookings and orders are saved safely and confirmed automatically when primary KobeOS returns.</div>}{error && <div className="rounded-xl bg-rose-50 border border-rose-200 text-rose-700 p-3 mb-4 text-sm">{error}</div>}{booking && <div className="rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 p-3 mb-4 text-sm font-bold">{booking}</div>}{reverseRequestId && <section className="rounded-[2rem] bg-white border p-5 sm:p-6 mb-6 shadow-sm"><div className="flex flex-col sm:flex-row sm:items-center gap-3"><div className="flex-1"><p className="text-xs font-black text-violet-700">HOTELS ARE COMPETING FOR YOU</p><h3 className="text-xl font-black">Your room offers</h3><p className="text-sm text-slate-500">Request {reverseRequestId.slice(0, 8)} · accept one offer to create the booking immediately.</p></div><button disabled={offersLoading} onClick={() => void loadOffers()} className="h-10 px-4 rounded-xl border font-black text-sm">{offersLoading ? 'Checking…' : 'Refresh offers'}</button></div><div className="mt-4 grid md:grid-cols-2 gap-3">{offers.map((offer) => <article key={offer.id} className="rounded-2xl border p-4"><div className="flex gap-3"><div className="flex-1"><b className="text-lg">{offer.hotel?.name ?? 'Hotel offer'}</b><p className="text-xs text-slate-500">{offer.hotel?.location ?? ''} · {offer.room?.type ?? 'Room'} {offer.room?.roomNumber ? `#${offer.room.roomNumber}` : ''}</p></div><strong className="text-violet-800">{money(Number(offer.totalPrice), offer.currency)}</strong></div>{offer.message && <p className="mt-3 text-sm text-slate-600">{offer.message}</p>}<p className="mt-2 text-[10px] text-slate-400">Expires {new Date(offer.expiresAt).toLocaleString()}</p><button onClick={() => void acceptOffer(offer)} disabled={offersLoading} className="mt-3 w-full h-10 rounded-xl bg-violet-800 text-white font-black disabled:opacity-40">Accept & book</button></article>)}{!offers.length && !offersLoading && <div className="md:col-span-2 rounded-2xl bg-violet-50 p-5 text-sm text-violet-700">No hotel offers yet. Keep this request open and refresh when hotels respond.</div>}</div></section>}{loading && !rows.length ? <div className="h-64 grid place-items-center"><Loader2 className="animate-spin" /></div> : <div className="space-y-6">{rows.map((row) => <article key={row.hotel.id} className="bg-white rounded-[2rem] border overflow-hidden shadow-sm"><div className="grid lg:grid-cols-[.8fr_1.2fr]"><img src={publicAssetUrl(row.profile.images[0]) || publicAssetUrl(row.availableRooms[0]?.imageUrl) || fallback} className="w-full h-64 lg:h-full object-cover" /><div className="p-5 sm:p-7"><div className="flex gap-3"><div className="flex-1"><h3 className="text-2xl font-black">{row.hotel.name}</h3><p className="text-sm text-slate-500 inline-flex items-center gap-1 mt-1"><MapPin className="h-4 w-4" />{row.hotel.location}</p></div><div className="text-right"><b className="inline-flex items-center gap-1 text-amber-600"><Star className="h-4 w-4 fill-current" />{row.profile.guestRating.toFixed(1)}</b><p className="text-[10px] text-slate-400">{row.profile.verifiedReviewCount} verified stays</p></div></div><p className="mt-3 text-sm text-slate-600">{row.profile.description}</p><div className="mt-3 flex flex-wrap gap-2">{row.profile.amenities.map((a) => <span key={a} className="px-2 py-1 rounded-lg bg-violet-50 text-violet-700 text-[11px] font-bold">{a}</span>)}</div><div className="mt-5 grid sm:grid-cols-2 gap-3">{row.availableRooms.slice(0, 6).map((room) => <button key={room.id} onClick={() => setSelected({ hotel: row, room })} className="text-left rounded-2xl border p-3 hover:border-violet-400 hover:bg-violet-50 transition"><div className="flex gap-3"><img src={publicAssetUrl(room.imageUrl) || fallback} className="h-16 w-20 rounded-xl object-cover" /><div><b className="block">{room.type}</b><span className="text-xs text-slate-500">Up to {room.capacity} guests</span><strong className="block text-violet-800 mt-1">{money(room.rate, room.currency)} <small className="font-normal">/ night</small></strong></div></div></button>)}</div>{row.foodAvailable && <button onClick={() => setFoodHotel(row)} className="mt-3 w-full h-11 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-black inline-flex items-center justify-center gap-2 text-sm"><Utensils className="h-4 w-4" /> Order food from this hotel</button>}<p className="mt-3 text-[10px] text-emerald-700 font-bold"><ShieldCheck className="inline h-3 w-3 mr-1" />Availability verified {new Date(row.verifiedAvailabilityAt).toLocaleTimeString()}</p></div></div></article>)}{!rows.length && !loading && <div className="py-24 text-center text-slate-400"><BedDouble className="h-12 w-12 mx-auto mb-3" /><b>No available rooms for this search</b></div>}</div>}</main>
...
function FoodOrderModal({ hotel, onClose }: { hotel: HotelResult; onClose: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      const res = await publicApi<{ id: string; pendingConfirmation?: boolean }>(`/public/hotel/${slug}/orders`, {
        method: 'POST',
        body: JSON.stringify({
      ...
      setPlaced(res.pendingConfirmation ? `Order ${res.id.slice(0, 8)} saved by the independent backup. It will be sent to ${hotel.hotel.name} automatically when primary KobeOS reconnects.` : `Order ${res.id.slice(0, 8)} sent to ${hotel.hotel.name}. They'll call ${contact.phone.trim()} to confirm.`);
