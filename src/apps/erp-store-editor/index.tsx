...
function PreviewProductCard({
  ...
  return (
    ...
    >
      ...
      >
        {product.imageUrl ? (
          <img src={assetUrl(product.imageUrl)} alt={product.name} className="absolute inset-0 h-full w-full object-cover" />
        ) : (
          <>
  ...
  if (settings.template === 'jerseys') {
    ...
    return (
      <JerseyStorefrontPreview
        ...
        products={products.map((product) => ({ ...product, currency: product.currency || 'TZS' }))}
      />
    );
  }
          ...
          <div className="w-full sm:max-w-sm rounded-2xl overflow-hidden border border-white/10" style={{ backgroundColor: bgStyleMap[settings.bgStyle] }} onClick={(e) => e.stopPropagation()}>
            <div className={`relative h-40 bg-gradient-to-br ${openProduct.gradient ?? productGradients[0]} opacity-90 flex items-center justify-center`}>
              {openProduct.imageUrl
                ? <img src={assetUrl(openProduct.imageUrl)} alt={openProduct.name} className="absolute inset-0 h-full w-full object-cover" />
...
export default function StoreEditor() {
      ...
      setCatalogProducts(
        (Array.isArray(rows) ? rows : [])
          ...
          .map((row, index) => ({
            ...
            imageUrl: row.imageUrl || (row as PosProductRow & { imageUrls?: string[] }).imageUrls?.[0] || undefined,
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const updated = await api<StoreSettings>('/store-settings', {
        method: 'PUT',
        body: JSON.stringify(editableSettings(settingsRef.current)),
      });
  ...
  const handlePublish = useCallback(async () => {
    ...
    try {
      ...
      if (!saved.isPublished && saved.domainSlug) {
      ...
      if (prov.mode === 'self-hosted' && prov.tunnelToken && bridge?.cloudflared?.persistToken) {
        await bridge.cloudflared.persistToken(prov.tunnelToken);
      }
    } catch (e) {
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-start gap-2 px-2.5 py-2 rounded-md border bg-white/[0.03] border-white/[0.06] text-[10px] text-white/50">
                    ...
                    <span className="flex-1">
                      Your online computer serves your store through Cloudflare
                      live at <span className="font-mono text-white/70">{settings.domainSlug || 'yourstore'}.kobeapptz.com</span>. Keep KobeOS running while customers shop.
                    </span>
                  </div>
                </div>
              )}
...
function editableSettings(settings: StoreSettings) {
  const keys: Array<keyof StoreSettings> = ["storeName", "tagline", "logoUrl", "faviconUrl", "customDomain", "bannerHeadline", "bannerSubtext", "bannerCta", "bannerBg", "bannerHeight", "bannerVisible", "primaryColor", "accentColor", "bgStyle", "cardStyle", "gridColumns", "productCardStyle", "showStock", "showCategoryBadge", "showQuickAdd", "productsPerPage", "headerStyle", "showSearch", "showCategoryNav", "showCartIcon", "footerText", "enableCategoryNav", "headingSize", "bodySize", "template", "siteConfig"];
