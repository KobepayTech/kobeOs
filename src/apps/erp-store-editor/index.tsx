...
import { ExternalLink, Globe2, Loader2, Monitor, Package, Plus, RefreshCw, Save, Search, Smartphone, Store, Trash2, X } from 'lucide-react';
        ...
        {loading ? <div className="grid place-items-center py-24 text-slate-500"><Loader2 className="h-6 w-6 animate-spin" /></div> : tab === 'store' ? (
          ...
          <div className="mx-auto max-w-6xl space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              <p className="text-xs text-slate-400">Live preview — reflects your unsaved changes as you type.</p>
              <div className="ml-auto flex gap-1 rounded-lg border border-white/10 p-1">
                {([['desktop', 'Desktop', Monitor], ['mobile', 'Mobile', Smartphone]] as const).map(([id, label, Icon]) => (
                  <button key={id} onClick={() => setDevice(id)} className={`inline-flex h-8 items-center gap-1.5 rounded-md px-2.5 text-xs font-black ${device === id ? 'bg-indigo-600 text-white' : 'text-slate-400'}`}>
                    <Icon className="h-3.5 w-3.5" />{label}
                  </button>
                ))}
              </div>
              {settings.publishedUrl && <a href={settings.publishedUrl} target="_blank" rel="noreferrer" className="inline-flex h-9 items-center gap-1 rounded-lg border border-white/10 px-3 text-xs font-black"><ExternalLink className="h-3.5 w-3.5" />Open live site</a>}
            </div>
            <div className="flex justify-center overflow-hidden rounded-2xl border border-white/10 bg-slate-900 p-3">
...
function CreateProductDialog({ onClose, onSaved }: { onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const save = async () => {
    ...
    try {
      await api('/pos/products', { method: 'POST', offlineFallback: false, body: JSON.stringify({ sku: form.sku.trim() || undefined, name: form.name.trim(), category: form.category.trim() || undefined, price: Number(form.price), stock: Number(form.stock || 0), currency: form.currency.trim() || 'TZS', imageUrl: form.imageUrl.trim() || undefined, description: form.description.trim() || undefined, active: true, sourceType: 'QUICK_ADD_IMPORT' }) });
      await onSaved();
    } catch (cause) { setError((cause as Error).message || 'Could not create product.'); }
    finally { setBusy(false); }
  };
  return <div className="fixed inset-0 z-[10000] grid place-items-center bg-black/70 p-4"><div className="w-full max-w-md rounded-2xl border border-white/10 bg-slate-900 p-4"><div className="flex items-center"><h2 className="text-lg font-black">Quick add product</h2><button onClick={onClose} className="ml-auto grid h-8 w-8 place-items-center rounded-lg border border-white/10"><X className="h-4 w-4" /></button></div><div className="mt-4 space-y-3"><Field label="SKU (optional)" value={form.sku} onChange={(value) => set('sku', value)} /><Field label="Product name" value={form.name} onChange={(value) => set('name', value)} /><Field label="Category" value={form.category} onChange={(value) => set('category', value)} /><div className="grid grid-cols-2 gap-3"><Field label="Price" type="number" value={form.price} onChange={(value) => set('price', value)} /><Field label="Stock" type="number" value={form.stock} onChange={(value) => set('stock', value)} /></div><Field label="Image URL" value={form.imageUrl} onChange={(value) => set('imageUrl', value)} /><Field label="Description" value={form.description} onChange={(value) => set('description', value)} />{error && <p className="text-sm text-rose-300">{error}</p>}<button onClick={() => void save()} disabled={busy || !form.name.trim() || !form.price} className="inline-flex h-11 w-full items-center justify-center rounded-xl bg-indigo-600 font-black disabled:opacity-50">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Create product'}</button></div></div></div>;
}
