...
export function ScreeningTab({ tenants, units }: { tenants: TenantLite[]; units: UnitLite[] }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="flex items-center justify-between"><h2 className="font-black text-slate-700 inline-flex items-center gap-2"><ShieldCheck className="h-4 w-4" /> Tenant screening</h2><span className="text-[10px] font-bold text-amber-600 bg-amber-50 rounded-lg px-2 py-1">Demo scoring — connect a provider for real reports</span></div>
    <Panel>
      {tenants.length ? <div className="space-y-2">{tenants.map((t) => { const r = reports[t.id]; return <div key={t.id} className="rounded-xl border border-slate-100 p-3"><div className="flex items-center gap-3"><b className="text-sm">{t.name}</b>{r && <StatusPill status={r.verdict} />}<div className="ml-auto flex items-center gap-2">{r ? <><button onClick={() => void decide(t.id, 'accepted')} className="text-[11px] font-black text-emerald-700">Accept</button><button onClick={() => void decide(t.id, 'rejected')} className="text-[11px] font-black text-rose-600">Reject</button></> : <Btn tone="ghost" busy={loadingReport === t.id} onClick={() => void screen(t.id)}><ShieldCheck className="h-3.5 w-3.5" /> Run screening</Btn>}</div></div>{r && <div className="mt-3 grid grid-cols-2 sm:grid-cols-5 gap-2 text-center">{([['Overall', r.overallScore, '/850'], ['Rental', r.rentalHistoryPct, '%'], ['Eviction', r.evictionHistoryPct, '%'], ['Criminal', r.criminalHistoryPct, '%'], ['Credit', r.creditHistoryPct, '%']] as const).map(([k, v, suffix]) => <div key={k} className="rounded-lg bg-slate-50 py-2"><div className="text-[10px] uppercase text-slate-400 font-bold">{k}</div><b className="text-sm">{v}{suffix}</b></div>)}</div>}</div>; })}</div> : <Empty title="No tenants to screen" body="Add tenants first; screening scores each tenant's rental, eviction, criminal and credit history." />}
    </Panel>
