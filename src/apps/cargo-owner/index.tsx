...
export default function CargoOwner() {
  ...
  const [view, setView] = useState<'parcels' | 'site'>('parcels');
    ...
    <div className="h-full overflow-y-auto bg-slate-950 p-5 text-white">
      <div className="mx-auto max-w-6xl space-y-4">
        ...
        <nav className="flex gap-1 border-b border-white/10">
          {(['parcels', 'site'] as const).map((id) => (
            <button key={id} onClick={() => setView(id)} className={`h-10 border-b-2 px-3 text-xs font-black capitalize ${view === id ? 'border-blue-300 text-blue-300' : 'border-transparent text-slate-500'}`}>{id === 'site' ? 'Website' : 'Parcels'}</button>
          ))}
        </nav>
        {view === 'site' ? <CargoSiteBuilder /> : <>
