...
function markdownToHTML(md: string): string {
  ...
  // Tables
  // Build the separator character class at runtime. The previous regex
  // literal looked like a Tailwind arbitrary utility to the source scanner,
  // which caused an invalid rule in the production stylesheet.
  const separatorChars = ['-', ':', '|', '\\s'].join('');
  const tablePattern = new RegExp(
    `\\|(.+)\\|\\n\\|([${separatorChars}]+)\\|\\n((?:\\|.+\\|\\n?)+)`,
    'g',
  );
  html = html.replace(tablePattern, (_match, header, _sep, rows) => {
