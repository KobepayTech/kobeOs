...
export default function OperatingControl() {
  ...
  const [dashboardLive, setDashboardLive] = useState<Record<string, { generatedAt: string; widgets: Array<Record<string, unknown>> }>>({});
  ...
  return (
    <div className="space-y-4">
      ...
      {section === 'dashboards' && <div className="space-y-3"><div className={card}><div className="flex gap-2"><input value={dashboardText} onChange={(e) => setDashboardText(e.target.value)} placeholder="e.g. revenue, expenses, rent and hotel occupancy" className="h-10 flex-1 rounded-xl border border-slate-300 px-3 text-sm" /><button disabled={!dashboardText.trim()} onClick={() => act('dashboard', async () => { await api('/ai/operating/dashboards', { method: 'POST', body: JSON.stringify({ prompt: dashboardText }) }); setDashboardText(''); })} className="rounded-xl bg-violet-600 px-4 text-xs font-bold text-white">Generate dashboard</button></div></div>{dashboards.map((dash) => { const live = dashboardLive[dash.id]; return <div key={dash.id} className={card}><div className="flex flex-wrap items-center gap-2"><LayoutDashboard className="h-5 w-5 text-violet-600" /><b className="flex-1">{dash.name}</b><button disabled={busy === `live-${dash.id}`} onClick={() => act(`live-${dash.id}`, async () => { const result = await api<{ generatedAt: string; widgets: Array<Record<string, unknown>> }>(`/ai/operating/dashboards/${dash.id}/live`); setDashboardLive((current) => ({ ...current, [dash.id]: result })); })} className="rounded-lg bg-violet-600 px-3 py-1.5 text-[10px] font-bold text-white">{busy === `live-${dash.id}` ? 'Refreshing…' : 'Refresh live'}</button></div><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{(live?.widgets || dash.widgets).map((widget, i) => <div key={String(widget.id || i)} className="rounded-xl bg-slate-50 p-3"><b className="text-xs">{String(widget.title || 'Metric')}</b><div className="mt-1 text-[10px] text-slate-500">{String(widget.source || '')} · {String(widget.visualization || '')}</div>{typeof widget.summary === 'string' && widget.summary && <div className="mt-2 text-xs font-semibold text-slate-800">{widget.summary}</div>}{typeof widget.confidence === 'number' && <div className="mt-1 text-[9px] text-slate-400">{Math.round(Number(widget.confidence) * 100)}% confidence</div>}</div>)}</div>{live?.generatedAt && <div className="mt-2 text-[9px] text-slate-400">Live data refreshed {new Date(live.generatedAt).toLocaleString()}</div>}</div>; })}</div>}
