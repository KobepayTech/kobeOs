...
export function MobileAssistant() {
  ...
  const refreshModels = async () => {
    ...
    try {
      ...
      if (!status.online) setModelError('The Kobe AI runtime on this node is offline.');
    } catch {
      setGateway(null);
      setModels([]);
  ...
  useEffect(() => { if (open) void refreshModels(); }, [open]);
            ...
            <div className="shrink-0 border-b border-white/10 px-3 py-2 text-white">
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {models.map((item) => <option key={item.name} value={item.name} className="text-black">{item.name}</option>)}
                </select>
