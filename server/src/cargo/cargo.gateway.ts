...
export type CargoEventKind = 'created' | 'status' | 'assignment';
...
export class CargoGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  private readonly logger = new Logger('CargoGateway');
  ...
  afterInit() {
    this.logger.log('Cargo WebSocket gateway ready on /cargo');
  }
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      // Scope every client to its owner so updates only reach that tenant.
      ...
      client.emit('cargo:error', { message: 'Unauthorized' });
  ...
  emitShipment(ownerId: string, shipment: Shipment, kind: CargoEventKind, previousStatus?: string) {
    this.server.to(`owner:${ownerId}`).emit('cargo:shipment', { kind, shipment, previousStatus });
  }
