...
export async function repairCatalogImages(repo: Repository<PosProduct>, ownerId: string, rows: PosProduct[]) {
  ...
  const assets = repo.manager.getRepository(MediaAsset);
  const found = await assets.find({ where: { ownerId, id: In(ids) } });
  const links = new Map<string, string>();
  for (const asset of found) {
    if (!asset.contentBinary || !['image', 'photo'].includes(asset.kind)) continue;
    if (!asset.publicToken) {
      ...
      const current = await assets.findOne({ where: { id: asset.id, ownerId } });
